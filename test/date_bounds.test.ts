...
void test('date partition bounds test', async (t) => {
  ...
  await t.test(
    'add files with dates that compare incorrectly as buffers',
    async () => {
  ...
  await t.test('verify manifest list bounds', async () => {
    ...
    const records: ManifestListRecord[] = await new Promise(
      (resolve, reject) => {
 ...
 resolve(results);
    ...
    assert(
      upperDays > lowerDays,
      'Upper days should be the greater than lower days'
    );

    // Verify that raw buffer comparison would give wrong answer
    const bufferCompare = Buffer.compare(upperBound, lowerBound);
    log('Buffer.compare result:', bufferCompare);
    log('Correct int32 comparison:', upperDays - lowerDays);
