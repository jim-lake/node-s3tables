...
void test('manifest compact force rewrite', async (t) => {
  const { namespace, name } = await setupTable(
    t,
    'test_ns_compact_force',
    'test_table_compact_force',
    [
  ...
  await t.test('add single file', async () => {
  ...
  await t.test('compact without forceRewrite returns unchanged', async () => {
  ...
  await t.test('compact with forceRewrite rewrites manifest', async () => {
    const result = await manifestCompact({
      tableBucketARN: config.tableBucketARN,
      namespace,
      name,
    ...
    log('Force rewrite result:', result);
    assert.strictEqual(result.changed, true);
    assert.strictEqual(result.outputManifestCount, 1);
  });
});
